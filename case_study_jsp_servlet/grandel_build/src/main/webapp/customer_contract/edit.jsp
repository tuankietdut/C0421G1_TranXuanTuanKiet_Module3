...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-lg-9 col-sm-12" >
            ...
            <h1 class="display-6 text-center">Edit Service</h1>
            <p style="color: darkred">${msg}</p>
            <form action="/customer_contract" method="post">

                <input type="hidden" name="id" value="${serviceObject.id}">
                <div class="mb-3">
                    ...
                    <select  onchange="onchangeService()"class="form-control" name="serviceType" id="changeService">
                        <c:forEach items="${serviceType}" var="serviceTypeObj">
                            <c:choose>
                                <c:when test="${serviceObject.typeService == serviceTypeObj.id}">
                                    <option value="${serviceTypeObj.id}" selected>${serviceTypeObj.name}</option>
                                </c:when>
                                <c:otherwise>
                                    <option value="${serviceTypeObj.id}">${serviceTypeObj.name}</option>
                                </c:otherwise>
                            </c:choose>
<%--                            <option value="${serviceTypeObj.id}">${serviceTypeObj.name}</option>--%>
                        </c:forEach>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="standard_room" name="standard_room" value="${serviceObject.standardRoom}">
                </div>

                <div class="mb-3">
                    <label for="description_other_convenience" class="form-label">Description_other_convenience</label>
                    <input type="text" class="form-control" id="description_other_convenience" name="description_other_convenience" value="${serviceObject.description}">
                </div>
